...
function NextGen(){
	...
	this.setStatusMessage = function(messages){
		...
		if(messages.constructor == Array){
			...
			if(NextGen.instance.statusElementId !== null){
				$('#' + NextGen.instance.statusElementId  + '.errors').show().html(messageHtml);
			}
			else{
            ...
            if(NextGen.instance.statusElementId !== null){
				$('#' + NextGen.instance.statusElementId + '.errors').html(messages);
            }
		}
	}	
		...
		$.ajax({
			...
			success: function(data){
				if(typeof(data.success) !== "undefined" && data.success == true) {
					...
					//Manage Gama account
					$.ajax({
						url: nextGenLoginInstance.manageGamaAccTemplate,
						data: {'isExtAuth' : true},
						success: function ( data ) {
	...
	/**
	* Create Gama account and authenticate, specific to Gamsutra
	*/
	this.manageGamaAccount = function ( operation ) {
		var url = null;
		if ( operation == 'create' ) {
			url = nextGenLoginInstance.createGamaAccTemplate;
			if ( $('#employerHidForm #emp_companyid').length && $('#employerHidForm #emp_companyid').val() ) {
				url = url + '?emp_companyid=' + $('#employerHidForm #emp_companyid').val();
			}
		}
		else if ( operation == 'update' ) {
			url = nextGenLoginInstance.updateGamaAccTemplate;
		}
...
NextGen.login = function(emailName, passwordName, remembermeName){
	if ( $('#TWLightBoxPanel #LoginForm').length ) {
		var email = $.trim($('#TWLightBoxPanel #LoginForm input[name='+emailName+']').val());
		var password = $('#TWLightBoxPanel #LoginForm input[name='+passwordName+']').val();
	}
	else {
	...
	if ( $('#LoginForm input[name="from"]').length > 0 && $('#LoginForm input[name="from"]').val()  ){
		nextGenLoginInstance.successlogin = $('#LoginForm input[name="from"]').val();
	}
...
jQuery.fn.nextGenInit = function(options) {	
	...
	this.createGamaAccTemplate = this.siteUrlPrefix+this.siteRedirectPrefix+'/registration/creategamaaccount';
	this.updateGamaAccTemplate = this.siteUrlPrefix+this.siteRedirectPrefix+'/registration/updategamaaccount';
	this.manageGamaAccTemplate = this.siteUrlPrefix+this.siteRedirectPrefix+'/registration/managegamaaccount';
	...
	this.welcomeCall = this.siteUrlPrefix+this.siteRedirectPrefix+'/welcome.php';
	...
	ng.loginCallback = function(userInfo){
		...
		//Specific for Gamasutra, if flag is set to create Gama account
		if(userInfo !== undefined && userInfo.createGamaAccount !== undefined && userInfo.createGamaAccount ) {
			ng.manageGamaAccount ( 'create' );
		}
...
function NextGenRegister (template, options) {
	...
	var bindForm = function(template) { 
		...
		if ( typeof(templateParts) !== "undefined" && typeof(templateParts.isSupplementary) !== "undefined" && templateParts.isSupplementary ) {
		...
		//Pre fill values if exist in URL
		if ( $('#employerHidForm #emp_emailid').length && $('#employerHidForm #emp_emailid').val() ) {
			$('#131, #confirmEmail').val($('#employerHidForm #emp_emailid').val());
		}
		if ( $('#employerHidForm #emp_firstname').length && $('#employerHidForm #emp_firstname').val() ) {
			$('#120').val($('#employerHidForm #emp_firstname').val());
		}
		if ( $('#employerHidForm #emp_lastname').length && $('#employerHidForm #emp_lastname').val() ) {
			$('#121').val($('#employerHidForm #emp_lastname').val());
		}
		if ( $('#employerHidForm #emp_jobtitle').length && $('#employerHidForm #emp_jobtitle').val() ) {
			$('#122').val($('#employerHidForm #emp_jobtitle').val());
		}
		if ( $('#employerHidForm #emp_org').length && $('#employerHidForm #emp_org').val() ) {
			$('#101').val($('#employerHidForm #emp_org').val());
		}
		if ( $('#employerHidForm #emp_jobtype').length && $('#employerHidForm #emp_jobtype').val() ) {
			if ( $('#employerHidForm #emp_jobtype').val() == 'employer' ) {
				$('#33773').val(64633);
			}
		}
				...
				$.ajax({
						...
						if (ct.indexOf('html') > -1) {
							...
							if(data.success == true){
								...
								//Specific for Gamasutra, if flag is set to create Gama account
								if ( data.manageGamaAccount !== undefined && data.manageGamaAccount ) {
									NextGen.instance.manageGamaAccount ( data.manageGamaAccount );
								}
		...
		nextGenLoginInstance.assetDownload({'redirectUrl':data.successfulLoginRedirect});
										...
										window.location = decodeURIComponent(data.successfulLoginRedirect);
                                                                ...
                                                                else if ( isAutoOpenLightBox () ) {
                                                                    window.location = decodeURIComponent('http://' + window.location.host);
                                                                }
								...
								if( typeof(templateParts.isSupplementary) !== "undefined" && isNGGatedContentPage() ) {
									...
									$('.welcome #lastName').text(data.lastName);
								}
							}
						}
				},
...
function isNGGatedContentPage(){
	if(typeof(ngGatedContentPage) !== "undefined" && ngGatedContentPage == 1)
		return true;
	else if ( $('#dynamiccomments') && $('#dynamiccomments').length )
		return true;
	else if ( $('#employerHidForm') && $('#employerHidForm').length )
		return true;
	else 
		return false;
}
...
function isAutoOpenLightBox () {
   ...
   var res = str.match(/sso\/profile.php/gi);
