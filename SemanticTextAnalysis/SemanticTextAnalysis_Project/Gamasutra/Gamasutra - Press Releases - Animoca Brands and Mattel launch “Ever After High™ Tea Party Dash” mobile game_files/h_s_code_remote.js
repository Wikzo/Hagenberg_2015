...
s.linkInternalFilters="javascript:,4gtrends.com,4gworld.com,adconf.com,allanalytics.com,allledlighting.com,bankandtechguide.com,banktech.com,banktechsubscriptions.com,bestofinterop.com,betterbanksystems.com,big-data-conference.com,bigtelecomevent.com,blackhat.com,business4better.org,callcentermagazine.com,canontradeshows.com,circuitlab.com,cloudconnectevent.com,createmarketingservices.com,csiannual.com,darkreading.com,datacenteracceleration.com,datasheets.com,ddj.com,ddjsubscriptions.com,designersofthings.com,designnews.com,dellpublicpartner.com,deusm.com,designcon.com,dlinkpartnercenter.com,drdobbs.com,driveforinnovation.com,e2conf.com,ebnonline.com,edn.com,eeliveshow.com,eeproductcenter.com,eetimes.com,eetimes.eu,eloqua.com,embedded.com,embeddedconf.com,enterprise2blog.com,ethernetsdnexpo.com,events-registration.com,fastspring.com,financetech.com,gamasutra.com,gamecareerguide.com,gamechoiceawards.com,gamedeveloperresearch.com,gamerbytes.com,gamesetwatch.com,gamesondeck.com,gdcaustin.com,gdcchina.com,gdceurope.com,gdconf.com,gdcnext.com,gdconline.com,gdc.tech.ubm.com,gdcvault.com,gdmag.com,gocsi.com,gocsiblog.com,gtec.ca,hdiconference.com,heavyreading.com,icmi.com,igf.com,igfmobile.com,incoming.com,indiegames.com,informationweek.com,informationweekconference.com,informationweek.de,informationweek.in,innovationgeneration.com,insuranceandtechguide.com,insurancetech.com,interop.com,inxpo.com,iotworld.com,itnint.com,latticepldchallenge.com,lightreading.com,mobilecommerceworld.com,mcommworld.com,mobileconnectevent.com,networkcomputing.com,networkcomputing.de,networkcomputing.in,nojitter.com,partbuyer.com,pharmaevolution.com,planetanalog.com,secure.events-registration.com,serviceprovideritreport.com, signummagazine.com,smartenterpriseexchange.com,smartenterprisemag.com,storageacceleration.com,techmarketing360.com,techonline.com,techonlineindia.com,techweb.com,techwebonlineevents.com,telcovisiononline.com,theenterprisecloudsite.com,themobilityhub.com,thinkhdi.com,tmworld.com,tmworldlaunchpad.com,towersummit.com,ubm.com,ubmcanon.com,ubmdesign.com,ubmelectronics.com,voicecon.com,wallstreetandtech.com,wallstreetandtechguide.com,wallstreetandtechsubscriptions.com,ubmfuturecities.com"
s.linkLeaveQueryString=false
s.linkTrackVars="events,products,eVar2,eVar4,eVar7,eVar24,prop7,prop4,eVar28,eVar29,eVar30"
s.linkTrackEvents="event2,event3,event7,event8,event9,event10,event11,event21,event23,event24,event25,event26,event43,event44,event45,event46,event47,event48,event49,event50"


/*Visitor Segmentation Config es20110210*/
s.varVisID="prop21"  //Will hold the VisitorID
s.varSegment="prop9"   //Will hold the segment
...
function s_doPlugins(s)
{
	...
	/* Plugin: getQueryParam 2.3 These are the Campaign Tracking Code values */
	s.campaign=s.getQueryParam('cid,kw,utm_campaign,utm_medium,utm_source,_mc,source,f_src',':');
	s.campaign=s.campaign.toLowerCase();

	/*Set evar47=s.campaign for last touch tracking code 3/17/14*/
	s.eVar47=s.getQueryParam('cid,kw,utm_campaign,utm_medium,utm_source,_mc,source,f_src',':');
	s.eVar47=s.eVar47.toLowerCase();
	
	/* Plugin: getQueryParam These are the internal tracking codes*/
 	s.prop18=s.getQueryParam('itc');
	s.prop18=s.prop18.toLowerCase();

	/* Plugin: getQueryParam elqCampaignId es11/8/13*/
	if(!s.eVar42)
	s.eVar42=s.getQueryParam('elqCampaignId');
	s.eVar42=s.eVar42.toLowerCase();
	
	/* Plugin: getQueryParam dwid es11/8/13*/
	if(!s.eVar43)
	s.eVar43=s.getQueryParam('dwid,ins',':');
	s.eVar43=s.eVar43.toLowerCase();

	/* Plugin: getQueryParam elqContactId es11/8/13*/
	if(!s.eVar44)
	s.eVar44=s.getQueryParam('elqContactId');
	s.eVar44=s.eVar44.toLowerCase();

	/*Set prop18=evar7 so can correlate custom insight traffic reports to internal campaign id*/
	if(!s.evar7)
	s.eVar7=s.prop18;

	/*Set evar40=s.channel for deusm*/
	s.eVar40=s.channel;

	/* Plugin: getQueryParam These are the source code and key code reports*/
 	s.eVar22=s.getQueryParam('k,sourceCode,keycode',':');
	s.eVar22=s.eVar22.toLowerCase();


	/* Plugin: getQueryParam 2.3 set s.prop1 to q= parameter for internal search q=google search results, queryText=verity, search_text=gama
	if(!s.prop1)
	s.prop1=s.getQueryParam('q,queryText,keyword,query,search,search_text',':');
	s.prop1=s.prop1.toLowerCase();

	/* Plugin: getQueryParam 2.3 set s.evar25 to f= parameter for formid */
	s.eVar26=s.getQueryParam('f,formID',':');

	/* Plugin: getNewRepeat 1.0 */
	s.prop17=s.getNewRepeat();


	/*Set evar17=prop17 so that Campaigns report values can be correlated to Visitor Status*/
	if(!s.evar17)
	s.eVar17=s.prop17;

	/*Set evar2=document.title*/
	s.eVar2=document.title;

	/*
 	* Set Value for Author Name | Headline report. 9sept2010
 	*/
	s.prop15=s.prop7 + " | " + s.prop4;

	/* Set Evar value to Author|Headline for correlation to Campaign IDs 9sept2010*/
	s.eVar24=s.prop15;

	/* Set Evar value to Author name for correlation to Campaign IDs 3/22/11ES*/
	s.eVar28=s.prop7;
...
/* Plugin: getVisualWebSiteOptimizer 3/22/13 */
try {
    if (typeof (_vis_opt_settings_loaded) == "boolean") {
        ...
        if (typeof (_vis_opt_comb_name[_combination]) != "undefined") {
           s.eVar38 = _vis_opt_experiment_id + ":" + _vis_opt_comb_name[_combination];
       }
    }
} catch (e) { }

/*
 * Plugin: getQueryParam 2.3 - this is to populate the (external) tracking code reports
 */
