...
public class GeoTransformationMatrix
{
	 ...
	 */
	public GeoTransformationMatrix(double a1, double a2, double a3, double b1, double b2, double b3, double c1,
			double c2, double c3)
	{
		matrix11 = a1;
		matrix12 = a2;
	...
	{
		return new GeoTransformationMatrix(matrix11 * _s, matrix12 * _s, matrix13 * _s, matrix21 * _s, matrix22 * _s,
				matrix23 * _s, matrix31 * _s, matrix32 * _s, matrix33 * _s);
	}
	...
	public Polygon multiply(Polygon _poly)
	{
		...
		{
			pointsX[i] = (int) (matrix11 * (double) _poly.xpoints[i] + matrix12 * (double) _poly.ypoints[i] + matrix13);
			pointsY[i] = (int) (matrix21 * (double) _poly.xpoints[i] + matrix22 * (double) _poly.ypoints[i] + matrix23);
		} // for i
	...
	{
		return new GeoTransformationMatrix(Math.cos(_rad), -Math.sin(_rad), 0, Math.sin(_rad), Math.cos(_rad), 0, 0, 0,
				1);
	}
	...
	public static double getZoomFactorX(Rectangle _map, Rectangle _win)
	{
		...
		return _map.width != 0 ? (double) _win.width / (double) _map.width : 1.0;
	}
	...
	public static double getZoomFactorY(Rectangle _map, Rectangle _win)
	{
		...
		return _map.height != 0 ? (double) _win.height / (double) _map.height : 1.0;
	}
	...
	public GeoTransformationMatrix invers()
	{
		...
		GeoTransformationMatrix matrix = new GeoTransformationMatrix(m11, m12, m13, m21, m22, m23, m31, m32, m33);
	...
	public double det()
	{
		return (matrix11 * matrix22 * matrix33) + (matrix12 * matrix23 * matrix31) + (matrix13 * matrix21 * matrix32)
				- (matrix11 * matrix23 * matrix32) - (matrix12 * matrix21 * matrix33)
	 ...
	 */
	public static GeoTransformationMatrix zoomToFit(Rectangle _map, Rectangle _win)
	{
		double scaleFactor = getZoomFactor(_map, _win);

		GeoTransformationMatrix translateA = GeoTransformationMatrix.translate(-_map.x, -_map.y);
		GeoTransformationMatrix scale = GeoTransformationMatrix.scale(scaleFactor);
		GeoTransformationMatrix mirrorX = GeoTransformationMatrix.mirrorX();
		GeoTransformationMatrix translateB = GeoTransformationMatrix.translate(0, _win.height);
	 ...
	 * @return Die Transformationsmatrix
	 */
	public static GeoTransformationMatrix zoomToFitNoMirror(Rectangle _map, Rectangle _win)
	{
		double scaleFactor = getZoomFactor(_map, _win);
	 ...
	 */
	public static GeoTransformationMatrix zoomToPoint(GeoTransformationMatrix _matrix, Point _p, double _scale)
	{
	 ...
	 * @return Die neue Transformationsmatrix
	 */
	public static GeoTransformationMatrix zoomToPoint(GeoTransformationMatrix _matrix, Point _p1, Point _p2,
			double _scale)
	{
		// auf den 0-Punkt verschieben ...
		GeoTransformationMatrix t1 = GeoTransformationMatrix.translate(-_p1.x, -_p1.y);
		// auf die richtige Groesse zoomen ...
		GeoTransformationMatrix z = GeoTransformationMatrix.scale((double) _scale);
		// zurueck zum Punkt verschieben ...
		GeoTransformationMatrix t2 = GeoTransformationMatrix.translate(_p2.x, _p2.y);
		// Matrizen in umgekehrter Reihenfolge zusammenmultiplizieren
		return t2.multiply(z.multiply(t1.multiply(_matrix)));
	}
	...
	{
		GeoTransformationMatrix matrixA = new GeoTransformationMatrix(7, 2, 4, 12, -4, 1, 23, 3, 3);
		...
		GeoTransformationMatrix matrix = GeoTransformationMatrix.zoomToFit(world, win);
